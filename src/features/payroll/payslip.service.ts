...
export class PayslipService {
  ...
  static async generatePayslips(
    ...
    const payrollCalculation =
      await PayrollCalculationService.calculateBatchPayroll(
        payPeriodId,
        employeeIds
      );
    ...
    for (const employeeCalculation of payrollCalculation.employees) {
      try {
        ...
        console.error(
          `Error generating payslip for employee ${employeeCalculation.employeeId}:`,
          error
        );
    ...
    const summary = {
      ...
      totalGrossPay: payslips.reduce((sum, p) => sum + (p.grossPay ?? 0), 0),
  ...
  */
   ...
   */
  static async getEmployeePayslipSummary(
    employeeId: string,
    year?: number
    ...
    return payslips.map((payslip) => {
      ...
      return {
        ...
        totalHours:
          Number(payslip.regularHours) + Number(payslip.overtimeHours),
   ...
   */
  static async getPayrollSummaryByPeriod(
    payPeriodId: string
  ): Promise<PayslipSummary[]> {
    const payslips = await prisma.payslip.findMany({
      where: { payPeriodId },
   ...
   */
  static async markPayPeriodAsPaid(
    payPeriodId: string
  ): Promise<{ message: string; payslipsCount: number }> {
    ...
    return {
      ...
      employee: payslip.employee
        ? {
            ...
            employeeNumber: payslip.employee.employeeNumber || undefined,
            payRate: payslip.employee.payRate
              ? Number(payslip.employee.payRate)
              : undefined,
  ...
  static async bulkUploadPayslipFiles(
    ...
    for (const file of files) {
      try {
        ...
        if (parts.length < 2) {
          errors.push(
            `Invalid filename format: ${fileName}. Expected: "Period - First Last.pdf"`
          );
          failedCount++;
          continue;
        }
        ...
        if (nameParts.length < 2) {
          errors.push(
            `Invalid name format in ${fileName}. Expected: "First Last"`
          );
          failedCount++;
          continue;
        }
        ...
        if (!employee) {
          errors.push(
            `Employee not found: ${firstName} ${lastName} (from ${fileName})`
          );
          failedCount++;
          continue;
        }
